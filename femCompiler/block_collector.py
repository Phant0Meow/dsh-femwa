...
def collect_blocks(
    ...
    # 完整替换走 FEMRunner._replace_prompt_vars（支持 @actor.attr 等属性访问）；
    # VarManager 上没有该方法（hasattr 会静默走简单分支导致 {@speaker.roles}
    # 这类表达式原样残留）。runner 参数是 collect_blocks 的调用方传入的。
    if runner is not None and hasattr(runner, '_replace_prompt_vars'):
        prompt_text = runner._replace_prompt_vars(raw_prompt)
    elif hasattr(var_manager, '_replace_prompt_vars'):
