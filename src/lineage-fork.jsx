...
// ── dsh-femwa fork 注入（2026-08-23）─────────────────────────────────────
// 本文件 fork 自 @deepseek-ai/dsh-client-ui-subagent/lib SubagentHeaderLineage.js
// （rc.2 快照），唯一改动=按名字约定隐藏 Fem 剧本机制产生的条目：
//   投影窗 id 前缀 'fem-proj-'（host projection.ts projectionId）
//   节点子代理 label 前缀 'fem-node-'（host subagent.ts runAiSubagent 的 start label）
...
function femwaHiddenId(id) { return typeof id === 'string' && id.startsWith('fem-proj-') }
function femwaHiddenLabel(label) { return typeof label === 'string' && label.startsWith('fem-node-') }
function femwaStripEntries(entries) {
  return entries.filter((entry) => !femwaHiddenId(entry.id) && !femwaHiddenLabel(entry.label))
}
function femwaStripSummaries(byId) {
  const out = {}
  for (const [id, summary] of Object.entries(byId ?? {})) {
    if (femwaHiddenId(id) || femwaHiddenLabel(summary?.displayTitle)) continue
    out[id] = summary
  }
  return out
}
...
function CatalogLoadingRows({ parentSessionId, summaries, level, t, }) {
    const children = Object.values(summaries).filter(summary => (summary.origin === 'subagent' && summary.parentId === parentSessionId)).filter((summary) => !femwaHiddenId(summary.id) && !femwaHiddenLabel(summary.displayTitle));
...
export function CatalogDropdown({ rootSessionId, currentSessionId, displayTitle, openTitle, variant, separator = false, showRunning = false, useSessions, openChild, refresh, setCatalogOpen, t, }) {
    ...
    const switcherDisplayTitle = currentEntry?.kind === 'child'
        ...
        : displayTitle;
    const healthy = femwaStripEntries(catalog?.entries.filter(entry => entry.kind === 'child') ?? []);
    const descendants = useMemo(() => indexSubagentDescendants(femwaStripSummaries(summaries)).get(rootSessionId) ?? NO_DESCENDANTS, [rootSessionId, summaries]);
