# Theme tokenization batch replace for femGen/src/*.jsx
# Maps hardcoded colors to var(--fem-*) CSS variables.
$ErrorActionPreference = 'Stop'

$map = [ordered]@{
  # 8-digit hex (must precede 6-digit)
...
# color: 'white' -> on-accent (button text); background: 'white' / fill="white" -> surface
$specialColor = @(
  @("color: 'white'", "color: 'var(--fem-on-accent)'"),
  @('color: "#fff"', 'color: "var(--fem-on-accent)"'),
  @("color: '#fff'", "color: 'var(--fem-on-accent)'"),
  @("background: 'white'", "background: 'var(--fem-surface)'"),
  @('fill="white"', 'fill="var(--fem-surface)"'),
  @('stroke="#3d5cf5"', 'stroke="var(--fem-primary)"'),
  @('fill="#3d5cf5"', 'fill="var(--fem-primary)"')
)

$dir = 'D:\myFiles\dsh\dsh-femwa\femGen\src'
$files = Get-ChildItem $dir -Filter *.jsx | Where-Object { $_.Name -notin @('main.jsx','femParser.jsx','femGenerator.jsx','graphBuilder.jsx','useCanvasInteraction.jsx') }

foreach ($f in $files) {
  $c = [System.IO.File]::ReadAllText($f.FullName)
  $orig = $c
  # special color combos first (color: '#fff' -> on-accent)
  foreach ($s in $specialColor) {
    $c = $c.Replace($s[0], $s[1])
  }
  $count = 0
