...
- **文件路径与文本值**：
  ...
  - **相对路径基于剧本文件所在目录解析**（不是项目根/CWD；func_code 默认位置已取消）；
    剧本未保存（纯文本直接运行）时相对路径不可用、只支持绝对路径——可先「导出 .FEMS」保存剧本再运行
  - 裸文本（无引号或普通引号） → 字面量字符串
  - `|` 后换行缩进 → 多行字面量字符串
...
- **上下文与scope**：[@角色名1, @soulname2]
  ...
  - **注意**：不写 scope ≠ 全可见——空 scope 的记录只对发言者本人与 meta.owner 可见；需要全员可见时请用 `scope: all` 或显式列出所有角色。
  - 开发者视角：
...
- **前端跟随**：femGen 动作编辑面板已支持 `@mind` 执行者类型（与 @ai/@human 一样编辑）；
