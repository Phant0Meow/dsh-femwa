#!/usr/bin/env python3
"""验证 fork 分支异常上报：goblin-mini（好）应跑通，goblin-mini-bad（@hero 未声明）应 flow_error。"""
...
for fname, expect in (("goblin-mini.fems", "flow_done"), ("goblin-mini-bad.fems", "flow_error")):
    print(f"\n===== {fname}（期望 {expect}）=====")
    ...
    for e in events:
        ev = e["event"]
        if ev in ("flow_error", "assign_result", "node_start"):
            print(f"  [{ev}] {json.dumps(e.get('data', {}), ensure_ascii=False)[:200]}")
    ok = ("flow_error" in types) if expect == "flow_error" else ("flow_done" in types and "flow_error" not in types)
